package assets

import (
	"embed"
)

var (
	// Fonts
	//go:embed fonts/Planewalker-38m6.ttf
	Magic_ttf []byte

	//go:embed fonts/PlanewalkerDings-Pl9d.ttf
	MagicSym_ttf []byte

	// Card data
	//go:embed card_info/scryfall_cards.json.zst
	Cards_json []byte

	//go:embed card_info/parsed_cards.json
	ParsedCards_json []byte

	// Basic land tiles
	//go:embed art/screens/world/land/Landtile.spr.png
	Landtile_png []byte

	// Grass and shrubs
	//go:embed art/screens/world/land/Land.spr.png
	Land_png []byte
	// Grass and shrubs shadows
	//go:embed art/screens/world/land/Sland.spr.png
	Sland_png []byte

	//go:embed art/screens/Icons.spr.png
	Icons_png []byte
	//go:embed art/screens/Iconb.spr.png
	Iconb_png []byte

	// Trees
	//go:embed art/screens/world/land/Land2.spr.png
	Land2_png []byte
	// Trees shadows
	//go:embed art/screens/world/land/Sland2.spr.png
	Sland2_png []byte

	//go:embed art/screens/world/land/Cstline_map.txt
	CstlineMap_txt []byte
	//go:embed art/screens/world/land/Cstline2.spr.png
	Cstline2_png []byte
	//go:embed art/screens/world/land/Cstline1.spr.png
	Cstline_png []byte

	//go:embed art/screens/world/land/Castles1.spr.png
	Castles1_png []byte

	//go:embed art/screens/world/land/Castles2.spr.png
	Castles2_png []byte

	// 6x2 city + shadow x2
	// Total 6x4
	//go:embed art/screens/world/land/Locatn01.spr.png
	Cities1_png []byte

	// 6x2 city + shadow x2
	// Total 6x4
	//go:embed art/screens/world/land/Locatn03.spr.png
	Dungeons_png []byte

	// Dungeon interior tilesets, 12 columns x N rows of 64x64 cells.
	// row 0 cols 0-7 = walkable floor variants
	// row 0 col 8    = treasure
	// row 0 col 9    = scroll/trivia
	// row 0 col 10   = dice
	// row 1 col 8    = wall / unpathed
	//go:embed art/screens/dungeons/Cavebkgd.pic.png
	DungeonBackground_png []byte
	//go:embed art/screens/dungeons/Dungeon1.spr.png
	Dungeon1_png []byte
	//go:embed art/screens/dungeons/Dungeon2.spr.png
	Dungeon2_png []byte
	//go:embed art/screens/dungeons/Dungeon3.spr.png
	Dungeon3_png []byte
	//go:embed art/screens/dungeons/Dungeonw.spr.png
	DungeonW_png []byte
	//go:embed art/screens/dungeons/Dungeonu.spr.png
	DungeonU_png []byte
	//go:embed art/screens/dungeons/Dungeonb.spr.png
	DungeonB_png []byte
	//go:embed art/screens/dungeons/Dungeonr.spr.png
	DungeonR_png []byte
	//go:embed art/screens/dungeons/Dungeong.spr.png
	DungeonG_png []byte

	// Random Encounters
	//go:embed art/screens/world/land/Locatn07.spr.png
	RandomEncounters_png []byte

	// World frame
	//go:embed art/screens/world/Advinter1024.pic.png
	WorldFrame_png []byte

	//go:embed art/screens/world/Worlds.spr.png
	WorldSpr_png []byte

	//go:embed art/screens/world/Questnew.spr.png
	QuestScroll_png []byte

	//go:embed art/screens/world/land/Roads.spr.png
	Roads_png []byte

	// Character sprites
	//go:embed art/screens/world/characters/*.png
	CharacterFS embed.FS

	//////////////////////
	// Visit City Screens
	//////////////////////

	//go:embed art/screens/city/City.png
	City_png []byte
	//go:embed art/screens/city/Village.png
	Village_png []byte
	//go:embed art/screens/city/Buycards.png
	BuyCards_png []byte
	//go:embed art/screens/city/Smbuybttn.png
	BuyCardsSprite_png []byte
	//go:embed art/screens/city/Smbuybttn_map.json
	BuyCardsSpriteMap_json []byte
	//go:embed art/screens/city/Wiseman3.pic.png
	Wiseman_png []byte

	//go:embed art/screens/Buybuttons.spr.png
	BuyButtons_png []byte

	//go:embed art/screens/Amsprite.spr.png
	Amsprite_png []byte

	//////////
	// MiniMap
	//////////

	//go:embed art/screens/mini_map/*.png
	MiniMapFS embed.FS
	//go:embed art/screens/mini_map/Ttsprite.spr.png
	MiniMapTerrSpr_png []byte
	//go:embed art/screens/mini_map/Mapback.pic.png
	MiniMapFrame_png []byte
	//go:embed art/screens/mini_map/Mapbttns.pic.png
	MiniMapFrameSprite_png []byte
	//go:embed art/screens/mini_map/Mapbttns_map.json
	MiniMapFrameSprite_json []byte

	// Rogues
	// Paths used to load rogue configs and images
	//go:embed art/screens/world/characters/*.png
	RogueSpriteFS embed.FS
	//go:embed art/rogues/*.png
	RogueVisageFS embed.FS
	//go:embed configs/rogues/*.toml
	RogueCfgFS embed.FS

	//go:embed configs/card_tiers.toml
	CardTiers_toml []byte

	//go:embed configs/quests/*.toml
	QuestCfgFS embed.FS

	////////////////////////
	// Duel screen sprites
	////////////////////////
	//go:embed art/screens/duel/*.png
	DuelFS embed.FS

	//go:embed art/screens/Statbutt.spr.png
	Statbutt_png []byte

	////////////////////////
	// Duel Ante backgrounds
	////////////////////////
	//go:embed art/screens/duel_ante/*.png
	DuelAnteFS embed.FS

	// Duel Ante UI sprites
	//go:embed art/screens/duel_ante/Prdfrmb.pic.png
	DuelAnteBorder_png []byte
	//go:embed art/screens/duel_ante/Prdfrmb_map.json
	DuelAnteBorderMap_json []byte

	//go:embed art/screens/duel_ante/Prdfrma.pic.png
	DuelAnteStats_png []byte
	//go:embed art/screens/duel_ante/Prdfrma_map.json
	DuelAnteStatsMap_json []byte
	//go:embed art/screens/duel_ante/Losedul2.pic.png
	DuelLoseBg_png []byte

	//go:embed art/screens/duel_win/Winbak01.pic.png
	DuelWinBg_png []byte
	//go:embed art/screens/duel_win/Endplak.pic.png
	DuelWinTextBox_png []byte

	////////////////////////
	// Edit Deck Screen
	////////////////////////
	//go:embed art/screens/edit_deck/Dekbar1.pic.png
	EditDeckBg_png []byte

	//go:embed art/screens/edit_deck/Dektile4.pic.png
	EditDeckTile_png []byte

	//go:embed art/screens/edit_deck/sprite_sheet.png
	EditDeckFilterSheet_png []byte
	//go:embed art/screens/edit_deck/sprite_sheet_hover.png
	EditDeckFilterSheetHover_png []byte
	//go:embed art/screens/edit_deck/sprite_sheet_pressed.png
	EditDeckFilterSheetPressed_png []byte

	//go:embed art/ui/MapBttns.png
	InfoBar_png []byte
	//go:embed art/ui/MapBttns_map.json
	InfoBarMap_json []byte

	//go:embed art/ui/Tradbut1.pic.png
	Tradbut1_png []byte

	//go:embed art/screens/start/Title.pic.png
	StartTitle_png []byte

	//go:embed art/screens/start/Menu2.pic.png
	StartMenu2_png []byte
	//go:embed art/screens/start/Menu2-Norm.pic.png
	StartMenu2Norm_png []byte
	//go:embed art/screens/start/Menu2-Hi.pic.png
	StartMenu2Hi_png []byte

	//go:embed art/screens/start/Menu3.pic.png
	StartMenu3_png []byte
	//go:embed art/screens/start/Menu3-But1.pic.png
	StartMenu3But1_png []byte
	//go:embed art/screens/start/Menu3but.pic.png
	StartMenu3but_png []byte

	// Random encounter backgrounds
	//go:embed art/screens/rand_encounter/*.png
	RandEncounterFS embed.FS

	// Card images
	//go:embed art/card/Cardbk_Gold.pic.png
	CardBlank_png []byte

	// Text
	//go:embed text/Advblocks.txt
	Advblocks_txt []byte

	// Audio
	//go:embed audio/sfx/* audio/bgm/*
	AudioFS embed.FS
)
