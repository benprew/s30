...
def get_sprite_offset_for_connection(from_edge, to_edge):
    ...
    # For hex tessellation, matching corners align:
    # A's from_corner aligns with B's from_corner
    # A's to_corner aligns with B's to_corner
    # Average both constraints for best pixel alignment
    offset_x = round(((x1_a - x1_b) + (x2_a - x2_b)) / 2)
    offset_y = round(((y1_a - y1_b) + (y2_a - y2_b)) / 2)

    return (offset_x, offset_y)


def create_connection_pair_visualization(
    sprite_a, sprite_b, sprite_id_a, sprite_id_b, edge_a, edge_b, similarity
):
    """
    ...
    draw.rectangle(
        [
            sprite_a_x - 2,
            sprite_a_y - 2,
            sprite_a_x + sprite_a.width + 2,
            sprite_a_y + sprite_a.height + 2,
        ],
        outline=(200, 0, 0),
        width=2,
    )
    draw.rectangle(
        [
            sprite_b_x - 2,
            sprite_b_y - 2,
            sprite_b_x + sprite_b.width + 2,
            sprite_b_y + sprite_b.height + 2,
        ],
        outline=(0, 0, 200),
        width=2,
    )
    ...
    draw.line(
        [sprite_a_x + x1_a, sprite_a_y + y1_a, sprite_a_x + x2_a, sprite_a_y + y2_a],
        fill=(255, 0, 0),
        width=3,
    )
    ...
    draw.line(
        [sprite_b_x + x1_b, sprite_b_y + y1_b, sprite_b_x + x2_b, sprite_b_y + y2_b],
        fill=(0, 0, 255),
        width=3,
    )
    ...
    # Label sprites
    draw.text(
        (sprite_a_x + 5, sprite_a_y + 5),
        f"({sprite_id_a[0]},{sprite_id_a[1]})",
        fill=(200, 0, 0),
    )
    draw.text(
        (sprite_b_x + 5, sprite_b_y + 5),
