...
func TestBuyCard_PurchaseLogic(t *testing.T) {
	...
	player := &domain.Player{Gold: 10, CardMap: make(map[int]int)}
	...
	if player.CardMap[cardIdx] != 1 {
		t.Errorf("Expected player to have 1 of card %d, got %d", cardIdx, player.CardMap[cardIdx])
	}
...
func TestBuyCard_NotEnoughGold(t *testing.T) {
	city := &domain.City{}
	city.CardsForSale = []int{0}
	player := &domain.Player{Gold: 2, CardMap: make(map[int]int)}
	cardIdx := city.CardsForSale[0]
	card := domain.CARDS[cardIdx]
	card.Price = 5
	...
	if player.CardMap[cardIdx] != 0 {
		t.Errorf("Expected player to have 0 of card %d, got %d", cardIdx, player.CardMap[cardIdx])
	}
