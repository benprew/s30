...
func loadImg(asset []byte, scale float64) *ebiten.Image {
	img, _, err := image.Decode(bytes.NewReader(asset))
	if err != nil {
		panic(fmt.Sprintf("Unable to load image: %s", err))
	}

	return elements.ScaleImage(ebiten.NewImageFromImage(img), scale)
}
...
func NewWinDuelScreen(cards []*domain.Card) *DuelWinScreen {
	...
	textBg := loadImg(assets.DuelWinTextBox_png, 1.0)
	...
	return &DuelWinScreen{
		cards:      cards,
		Background: loadImg(assets.DuelWinBg_png, 1.6),
		textbox:    tb,
	}
}
