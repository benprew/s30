* Edit Deck Screen
** Mapped Sprite Sheet
Write this function in game/sprites/loader.go. including this function comment
<Comment Start>
Load a mapped sprite sheet using names and dimensions in spriteMap.
Args:
...
Signature:
LoadMappedSprite(spriteData []byte, spriteMap []byte) map[string]*ebiten.Image
** Scrollable list UI
Create a new UI element "ScrollableList".
...
The buttons are "fwd" and "ff". Fwd buttons move the list one item at a time, and fwd moves the list a full page.
...
Scrolling is accomplished by clicking on buttons to either move 1 item or 1 screen at a time. The buttons are in the Sprite sheet assets/art/ui/MapBttns.png. You can load it with the sprite map LoadMappedSprite in game/ui/imageutil/loader.go

See also assets/art/ui/MapBttns_map.json for the mapping

Include the comment as a function comment

** Collection cards
Build an edit_deck screen. This screen is responsible for allowing players to edit their decks.

Look at edit_deck.png to see what the screen should look like.
- On the bottom is the list of cards the player owns. A player can add a card to their deck by double-clicking on it or clicking and dragging the card into the deck area.
- Create a ScrollableList for this. Use assets/art/sprites/edit_deck/Winbk_Spellchain.pic.png for the background. Create an entry in assets/embed.go for this file.
- The list contains the player's card collection
- The dimensions of the collections portion of the screen is 1024x275 and is at the bottom of the screen.

** Card Magnifier
In the edit_deck screen, on the left-hand side of the screen is the card magnifier. It shows the full size card. Card images are 300x419. When a player hovers over a card it should show the full card in the magnifier. The card stays magnified until a player hovers over another card.

For the scroll list the id of the button will be the name of the card, can use FindCardsByName() and c.CardID() to identify the specific card.
